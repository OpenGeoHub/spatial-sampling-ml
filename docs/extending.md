...
- Using the probability of exceeding threshold mapping accuracy to generate sampling,

In both cases 2nd round points can be then added to the original training dataset and the 
model for the area can then be refitted (this procedure is also referred to as _"re-analysis"_). 
...
study area and including some randomization.
...
could be considered _the shortest path_ to increasing the mapping accuracy.
