...
cookfarm.grid_df <- raster(cookfarm.grid["pr.VW"]) %>%  as.data.frame(xy=T) %>% na.omit()
cookfarm_profiles <-cookfarm$profiles[,c("Easting","Northing")]

ggplot() +
  geom_raster(data = cookfarm.grid_df , aes(x = x, y = y, fill = pr.VW)) +
  geom_point(data = cookfarm_profiles, aes(x=Easting,y=Northing ), shape="+", size=3)+
  scale_fill_gradientn(colours  = rev(bpy.colors()), name="") +
  coord_sf()+
  ggtitle("Predicted VW for 2012-07-30 and depth -0.3 m") +
