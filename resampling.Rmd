...
eberg.xy_df <- as.data.frame(eberg.xy, xy=T)

ggplot() +
  geom_raster(data = eberg_spc_pred , aes(x = x, y = y, fill = PC1)) + 
  geom_point(data = eberg.xy_df, aes(x=X,y=Y), shape="+", size=2)+
  scale_fill_gradientn(colours  =SAGA_pal[[1]], name="") + 
  coord_sf()+
  ggtitle("All sampling points available for \nEbergotzen case study") +
...
sub.lst_df <- as.data.frame(sub.lst[[1]]$grid, xy=T)
l1 <- as.data.frame(sub.lst[[1]]$subset)

ggplot() +
  geom_raster(data = sub.lst_df , aes(x = s1, y = s2), fill = "grey") +
  geom_point(data = l1, aes(x=X,y=Y), shape="+", size=2)+
  coord_sf()+
  ggtitle("Resampling original points \nusing `sample.grid` function") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
```
...
eberg_grid25_df <- stack(eberg_grid25[c("pred.cly.rf", "pred.cly.erf")]) %>% 
  as.data.frame(xy=T) %>% gather(key=layer, value = value, -x, -y)

ggplot() +
  geom_raster(data = eberg_grid25_df , aes(x = x, y = y, fill = value)) + 
  scale_fill_gradientn(colours  = SAGA_pal[[1]], name="") + 
  facet_wrap(layer~.)+
  coord_sf()+
  theme_minimal()+  xlab("Westing")+ ylab("Northing")+
...
iprob.all_prob <- raster(iprob.all$prob) %>%  as.data.frame(xy=T)
iprob.all_obs  <- as.data.frame(iprob.all$observations, xy=T)

ggplot() +
  geom_raster(data = iprob.all_prob , aes(x = x, y = y, fill = iprob)) + 
  geom_point(data = iprob.all_obs, aes(x=X,y=Y), shape="+", size=2)+
...
r.sp_df <-  as.data.frame(r.sp,xy=T)
eberg.xy_df <- as.data.frame(eberg.xy, xy=T)

ggplot() +
  geom_raster(data = r.sp_df , aes(x = x, y = y, fill = gid)) + 
  geom_point(data = eberg.xy_df, aes(x=X,y=Y), shape="+", size=2)+
  scale_fill_gradientn(colours  = terrain.colors(10), name="") + 
  coord_sf()+
  ggtitle("500 m grid for spatial resampling") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
...
eberg_grid25_df <- stack(eberg_grid25[c("pred.cly.eml", "pred.cly.erf")]) %>% 
  as.data.frame(xy=T) %>% gather(key=layer, value = value, -x, -y)
...
eberg_grid25_df <- raster(eberg_grid25[c("rmspe.cly.eml")]) %>%  as.data.frame(xy=T)
eberg.sp_df  <- as.data.frame(eberg.sp, xy=T)

ggplot() +
  geom_raster(data = eberg_grid25_df , aes(x = x, y = y, fill = rmspe.cly.eml)) + 
  geom_point(data = eberg.sp_df, aes(x=X,y=Y), shape="+", size=2)+
  scale_fill_gradientn(colours  = SAGA_pal[[16]], name="") + 
  coord_sf()+
  ggtitle("Occurrence probability") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
```
...
edgeroi.grids_pred <- raster(edgeroi.grids@predicted[1]) %>%  as.data.frame(xy=T)
edgeroi.sp_df  <- as.data.frame(edgeroi.sp, xy=T) %>% select(LONGDA94,LATGDA94)

ggplot() +
  geom_raster(data = edgeroi.grids_pred , aes(x = x, y = y, fill = PC1)) + 
  geom_point(data = edgeroi.sp_df, aes(y=LATGDA94,x=LONGDA94 ), shape="+", size=3)+
  scale_fill_gradientn(colours  = rev(terrain.colors(10)), name="") + 
  coord_sf()+
  ggtitle("The Edgeroi dataset ") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
...
subE.lst_df <- as.data.frame(subE.lst[[1]]$grid, xy=T)
lE1 <- as.data.frame(subE.lst[[1]]$subset, xy=T)

ggplot() +
  geom_raster(data = subE.lst_df , aes(x = s1, y = s2), fill = "grey") +
  geom_point(data = lE1, aes(x=LONGDA94,y=LATGDA94), shape="+", size=2)+
  coord_sf()+
  ggtitle("Resampling original points using `sample.grid` function \nfor the Edgeroi dataset") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
```
...
edgeroi.grids.100m_df <- stack(edgeroi.grids.100m[c("rf_soc_5cm", "eml_soc_5cm")]) %>% 
  as.data.frame(xy=T) %>% gather(key=layer, value = value, -x, -y)
l.pnts <- as.data.frame(edgeroi.sp, xy=T)

ggplot() +
  geom_raster(data = edgeroi.grids.100m_df , aes(x = x, y = y, fill = value)) + 
  scale_fill_gradientn(colours  = SAGA_pal[[1]], name="") +
  geom_point(data = l.pnts, aes(x=LONGDA94,y=LATGDA94), shape="+", size=2)+
  facet_wrap(layer~.)+
  coord_sf()+
  theme_minimal()+  xlab("Westing")+ ylab("Northing")+
  theme(panel.spacing = unit(2, "lines"))
...
edgeroi.grids.100m_df <- raster(edgeroi.grids.100m[c("rmspe.soc.eml")]) %>%  as.data.frame(xy=T)

ggplot() +
  geom_raster(data = edgeroi.grids.100m_df , aes(x = x, y = y, fill = rmspe.soc.eml)) + 
  geom_point(data = edgeroi.sp_df, aes(y=LATGDA94,x=LONGDA94 ), shape="+", size=3)+
  scale_fill_gradientn(colours  = SAGA_pal[[16]], name="") + 
  coord_sf()+
  ggtitle("Prediction errors for the soil organic carbon content based \non the forestError package and Ensemble ML. ") +
  theme_minimal()+  xlab("Westing")+ ylab("Northing")
```
