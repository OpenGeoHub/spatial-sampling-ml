...
```{r doc status, echo=, results="asis"}
status("drafting")
```

```{r packages loading, message=FALSE, include=FALSE, results='hide'}
...
```{r PCA, fig.width=9, fig.height=3, fig.cap = "Principal Components derived using Ebergotzen dataset."}
...
ggplot() +
  ...
  theme_void()+  xlab("Westing")+ ylab("Northing")
...
ml.prob_gg <- ml.prob %>%  as.data.frame(xy=T)
...
rnd.lhs_coords <- eberg_spc_pred[rnd.lhs,]
