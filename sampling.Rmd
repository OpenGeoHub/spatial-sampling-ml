...
```{r scheme-d1, echo=FALSE, fig.cap="Example of D1 design: (left) D1 design in 2D feature space, (right) D1 design is optimal only for linear model, if the model is curvilinear, it is in fact the worse design than simple random sampling.", out.width="90%"}
knitr::include_graphics("./img/Fig_D1_design_scheme.jpg")
```
...
```{r PCA, fig.width=10, fig.height=3.5, fig.cap = "Principal Components derived using Ebergotzen dataset."}

compare.map(df = stack(eberg_spc@predicted[1:3]), title = "Ebergotzen PC1, PC2 and PC3")
...
rnd_pred <- as.data.frame(rnd, xy = TRUE)

```

```{r eberg-srs, echo = TRUE, out.width = "80%", fig.cap = "An example of a Simple Random Sample (SRS)."}

plot.map(df = raster(eberg_spc@predicted[1]), points = rnd_pred, title = "Simple Random Sample (SRS)")
...
plot.map(df = ml.prob, points = rnd_pred, fill = terrain.colors(10),
         title = "SRS occurrence probability using maxlike")
...
plot_hex(df=eberg_spc@predicted@data[,1:2], pts=ov2.rnd, title = 'PCA Ebergotzen actual')
```
...
```{r, cache=TRUE}
library(clhs)
rnd.lhs = clhs::clhs(eberg_spc@predicted[1:4], size=100, iter=100, progress=FALSE,simple=FALSE)
```

This actually implements the so-called _“Conditioned LHS”_ [@minasny2006conditioned], 
...
plot.map(df=raster(eberg_spc@predicted[1]), 
         points = as.data.frame(rnd.lhs$sampled_data@coords), 
         title="Latin Hypercube Sample (LHS)")
...
plot_hex(df=eberg_spc@predicted@data[,1:2], pts=as.data.frame(rnd.lhs$sampled_data@data), title = 'PCA Ebergotzen LHS')
...
plot.map(df=raster(eberg_spc@predicted[1]), 
         points = as.data.frame(rnd.fscs), 
         title="Feature Space Coverage Sampling (FSCS)" )
...
plot_hex(df=eberg_spc@predicted@data[,1:2], pts=eberg_spc@predicted@data[units,], title = 'PCA Ebergotzen FSCS')
...
a= eberg_spc@predicted@data[,1] - min(eberg_spc@predicted@data[,1]) +1

x= eberg_spc@predicted@data[,1:4] - min(eberg_spc@predicted@data[,1:4])+1
x = cbind(eberg_spc@predicted@coords,x)
prob <- probabilities(a,n=100)

sampled <- SamplingBigData::lpm2_kdtree(prob,x)

sbd_pnt = eberg_spc@predicted@coords[sampled,]


```


```{r eberg-fscs, echo = TRUE, out.width = "80%", fig.cap = "An example of Spatially Balanced Sampling (FSCS)."}

plot.map(df=raster(eberg_spc@predicted[1]), 
         points = as.data.frame(sbd_pnt), 
         title="Spatially Balanced Sampling (SBS)" )
...
plot_hex(df=eberg_spc@predicted@data[,1:2], pts=eberg_spc@predicted@data[sampled,], title = 'PCA Ebergotzen SBS')
